...
# Initialize OpenAI client (will pick up api_key from openai.api_key)
client = OpenAI()
...
def simulate_and_draft(rosters, id_to_name, id_to_pos,
    ...
    for owner, roster_ids in rosters.items():
        ...
        breakdown[owner] = {"protected": [id_to_name[p] for p in protected],
...
def ai_protect(roster_ids, id_to_name, id_to_pos, max_protect, pos_caps):
    ...
    prompt = (
        ...
        f"Select exactly {max_protect} players to PROTECT based on performance, upside, and scarcity. "
        f"Respect these max losses per position: {json.dumps(pos_caps)}. "
        "Respond with a JSON array of protected player names."
    )
    try:
        resp = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role":"system","content":"You pick fantasy roster protections."},
    ...
    protected_ids = [name_to_id[n] for n in names if n in name_to_id]
    return protected_ids[:max_protect] or roster_ids[:max_protect]
...
def ai_draft(pool_ids, id_to_name, num_teams, picks_per_team, draft_format):
    ...
    prompt = (
        ...
        f"Draft {picks_per_team} rounds for {num_teams} teams in {draft_format} format. "
        "Balance needs and value. Respond with JSON mapping team names to player name arrays."
    )
    try:
        resp = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role":"system","content":"You run drafts."},
                      {"role":"user","content":prompt}],
            temperature=0.2
        )
        mapping = json.loads(resp.choices[0].message.content)
    except Exception:
        mapping = {f"Expansion Team {i+1}": [] for i in range(num_teams)}
        for idx, pid in enumerate(pool_ids[:num_teams * picks_per_team]):
            team = (mapping.keys()[idx % num_teams] if draft_format == "Linear"
                    else (list(mapping.keys()) if (idx//num_teams)%2==0 else list(reversed(list(mapping.keys()))))[idx%num_teams])
            mapping[team].append(id_to_name.get(pid, pid))
    name_to_id = {v:k for k,v in id_to_name.items()}
    return {team: [name_to_id[n] for n in names if n in name_to_id]
...
if league_id:
    ...
    if run:
        if not use_ai:
            ...
            if invalid:
                st.error(
                    f"Each team must protect exactly {max_protect} players! Check: {', '.join(id_to_team.get(o,f'Owner {o}') for o in invalid)}"
                )
                st.stop()
        ...
        tab1,tab2,tab3,tab4 = st.tabs(["Team Breakdown","Draft Pool","Draft Results","Expansion Rosters"])
        ...
        with tab4:
            for team,pids in picks_by_team.items():
                st.subheader(team)
                df4 = pd.DataFrame([{"Player":id_to_name.get(pid,pid),"Position":id_to_pos.get(pid,"UNK")} for pid in pids])
                st.table(df4)
                st.download_button(
                    f"Download {team} Roster CSV",df4.to_csv(index=False),f"{team.replace(' ','_')}_roster.csv"
                )
